...
bool ReadTraceeMemoryProcessVm(pid_t pid, uint64_t remote_address, void* out, size_t len) {
    iovec local_iov{out, len};
    iovec remote_iov{reinterpret_cast<void*>(remote_address), len};
    const ssize_t copied = process_vm_readv(pid, &local_iov, 1, &remote_iov, 1, 0);
    return copied == static_cast<ssize_t>(len);
}

bool WriteTraceeMemoryProcessVm(pid_t pid, uint64_t remote_address, const void* data, size_t len) {
    iovec local_iov{const_cast<void*>(data), len};
    iovec remote_iov{reinterpret_cast<void*>(remote_address), len};
    const ssize_t copied = process_vm_writev(pid, &local_iov, 1, &remote_iov, 1, 0);
    return copied == static_cast<ssize_t>(len);
}

bool ReadTraceeCStringPtrace(pid_t pid, uint64_t remote_address, size_t max_len, std::string* out) {
    out->clear();
    out->reserve(max_len);
    size_t copied = 0;
    while (copied < max_len) {
        ...
        for (size_t i = 0; i < sizeof(long) && copied < max_len; ++i, ++copied) {
            const char ch = static_cast<char>((static_cast<unsigned long>(word) >> (i * 8U)) & 0xFFU);
            if (ch == '\0') {
                return true;
            }
            out->push_back(ch);
...
bool WriteTraceeMemoryPtrace(pid_t pid, uint64_t remote_address, const void* data, size_t len) {
...
bool ReadTraceeCString(pid_t pid, uint64_t remote_address, size_t max_len, std::string* out) {
    ...
    if (ReadTraceeMemoryProcessVm(pid, remote_address, buffer.data(), buffer.size())) {
        const char* terminator = static_cast<const char*>(memchr(buffer.data(), '\0', buffer.size()));
